...
# Skip the entire initializer when OAuth is disabled in yeti_web.yml. This
# means yeti can boot without the doorkeeper gem installed (and without
# applying the doorkeeper migration) as long as oauth.enabled is false.
return unless YetiConfig.oauth&.enabled
...
Doorkeeper.configure do
  ...
  # Who is authorizing this OAuth flow? Reuses the existing Devise/AA login —
  # local password or OIDC, whichever the user normally uses to access yeti.
  # If not signed in, redirect to the standard admin login; Devise picks up
  # the stored location after sign-in and bounces back to /oauth/authorize.
  resource_owner_authenticator do
    current_admin_user || (
      flash[:notice] = 'Please sign in to authorize this application.'
      store_location_for(:admin_user, request.fullpath)
  ...
  # Custom model classes so the tables live in the gui schema. See
  # app/models/oauth_*.rb — thin wrappers that include the Doorkeeper mixins
  # and set self.table_name = 'gui.oauth_*'.
  ...
  # Short access token TTL so offboarding (admin_user.enabled = false) propagates
  # within ~1h — refresh requires going through the resource_owner check again.
  ...
  # Require HTTPS for redirect URIs except for loopback hosts — MCP clients
  # (Claude Code, Cursor) bind a local HTTP callback during the authorization
  ...
  #
  # Only authorization_code with PKCE — no client_credentials (no machine
  # actors), no password (insecure), no implicit (deprecated).
  ...
  # Block token issuance (auth-code AND refresh) for disabled AdminUsers.
  # The MCP server also rejects access tokens for disabled admins at request
