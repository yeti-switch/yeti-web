...
Rails.application.routes.draw do
  ...
  if YetiConfig.oauth&.enabled
    # skip_controllers :applications — clients are managed through the AA "OAuth
    # Applications" page, and leaving Doorkeeper's own UI mounted would claim the
    # oauth_application(s) route helper names that page needs.
    use_doorkeeper do
      skip_controllers :applications
    end
    # Must stay above use_doorkeeper_openid_connect: the OIDC gem claims
    # /.well-known/oauth-authorization-server too, and its version of that
    # document has no registration_endpoint. First route wins.
    ...
    namespace :rest do
      with_options defaults: { format: :json } do |api|
        ...
        namespace :customer do
          namespace :v1 do
            jsonapi_resources :accounts, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :rateplans, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :rates, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :outgoing_numberlists, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :outgoing_numberlist_items, only: %i[index show create update destroy] do
              jsonapi_relationships only: :show
            end
            jsonapi_resource :check_rate, only: %i[create] do
              jsonapi_relationships only: :show
            end
            ...
            jsonapi_resources :networks, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :network_types, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :network_prefixes, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :chart_active_calls, only: %i[create] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :chart_originated_cps, only: %i[create] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :payments, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :cryptomus_payments, only: %i[create show] do
              jsonapi_relationships only: :show
            end
            ...
            jsonapi_resources :countries, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :services, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :transactions, only: %i[index show] do
              jsonapi_relationships only: :show
            end
            jsonapi_resources :phone_systems_sessions, only: %i[create] do
              jsonapi_relationships only: :show
            end
