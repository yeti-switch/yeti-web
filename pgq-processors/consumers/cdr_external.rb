...
class CdrExternal < Pgq::ConsumerGroup
  AVAILABLE_HTTP_METHODS = [:post, :put, :get, :patch]

  @consumer_name = 'cdr_external'
  ...
  def config
    ...
    path = File.join File.dirname(__FILE__), '../../config/cdr_external.yml'
  ...
  def permit_field_for(event)
    permitted_field = config['cdr_fields']
  ...
  def send_event_to_external(event)
    ...
    response = RestClient.try(method, config['url'], *make_request_params(permit_field_for(event)))
