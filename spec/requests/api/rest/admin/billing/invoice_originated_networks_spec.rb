...
RSpec.describe Api::Rest::Admin::Billing::InvoiceOriginatedNetworksController, type: :request, bullet: [:n] do
  include_context :json_api_admin_helpers, type: :'invoice-originated-networks', prefix: 'billing'

  describe 'GET /api/rest/admin/invoice-originated-networks' do
    subject do
    ...
    let!(:records) do
      [
        *create_list(:invoice_originated_network, 2, :filled, invoice: invoices.first),
        *create_list(:invoice_originated_network, 2, :success, invoice: invoices.second),
        *create_list(:invoice_originated_network, 2, invoice: another_invoice)
      ]
    end
    ...
    it_behaves_like :json_api_check_pagination do
      let(:records) do
        ...
        Array.new(records_qty) do |i|
          create(:invoice_originated_network, invoice: invoices[i])
        end
      end
    ...
    context 'with filters' do
      ...
      #     {
      #       invoices.first.id => create_list(:invoice_originated_network, 2, invoice: invoices.first).map(&:id),
      #       invoices.second.id => create_list(:invoice_originated_network, 2, invoice: invoices.second).map(&:id),
      #       another_invoice.id => create_list(:invoice_originated_network, 2, invoice: another_invoice).map(&:id)
      #     }
      #   end
      # end
      ...
      it_behaves_like :jsonapi_filters_by_foreign_key, :invoice_account_id do
        let(:foreign_keys_to_ids) do
          {
            accounts.first.id => create_list(:invoice_originated_network, 2, invoice: invoices.first).map(&:id),
            accounts.second.id => create_list(:invoice_originated_network, 2, invoice: invoices.second).map(&:id),
            another_account.id => create_list(:invoice_originated_network, 2, invoice: another_invoice).map(&:id)
          }
        end
      end
    end
  ...
  describe 'GET /api/rest/admin/invoice-originated-networks/{id}' do
    subject do
      get json_api_request_path, params: json_api_request_query, headers: json_api_request_headers
    end
    ...
    let!(:record) do
      create(:invoice_originated_network, :success, invoice:)
    end
