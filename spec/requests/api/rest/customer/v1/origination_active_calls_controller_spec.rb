...
RSpec.describe Api::Rest::Customer::V1::OriginationActiveCallsController do
  ...
  include_context :json_api_customer_v1_helpers, type: 'origination-active-calls'
  let(:auth_headers) { { 'Authorization' => json_api_auth_token } }

  describe 'GET /api/rest/customer/v1/origination-active-calls' do
    subject do
      get '/api/rest/customer/v1/origination-active-calls', params: query, headers: auth_headers
    end
    ...
    let(:query) do
      { 'account-id': account.uuid, 'from-time': from_time }
    end
