...
RSpec.describe Jobs::DialpeerRatesApply, type: :cron_job do
  describe '#start!' do
    ...
    let!(:another_destination) { FactoryGirl.create(:destination, rateplan: rate_plan) }
    let!(:skipped_destination) { FactoryGirl.create(:destination, rateplan: rate_plan) }
    let!(:next_rate) do
      FactoryGirl.create :destination_next_rate,
