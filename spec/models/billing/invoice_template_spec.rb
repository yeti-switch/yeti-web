...
RSpec.describe Billing::InvoiceTemplate do
  ...
  it 'defaults filename_template so a new template always names its documents' do
    expect(described_class.new.filename_template).to eq('invoice-{{invoice.reference}}')
  end

  # NOT NULL alone would let '' through, which yeti-pdf reads as "no filename
  # template" — the document would come back nameless.
  it 'is invalid with a blank filename_template' do
    template = described_class.new(name: 'blank-name', html_template: '<p>x</p>', filename_template: '')
    expect(template).not_to be_valid
    expect(template.errors[:filename_template]).to be_present
  end
end
