...
RSpec.describe 'Edit Dialpeer', js: true do
  ...
  context 'with filled routing tags' do
    let(:fill_form!) do
      ...
      fill_in_tom_select 'Routing tags', with: routing_tags[3].name, multiple: true
      fill_in_tom_select 'Routing tags', with: routing_tags[1].name, multiple: true
  ...
  context 'with different vendor and account' do
    ...
    let!(:record) do
      FactoryBot.create(:dialpeer, record_attrs)
    end
    let(:record_attrs) do
      {
        vendor: old_vendor,
    ...
    context 'when gateway_group was filled' do
      let(:record_attrs) do
        super().merge gateway: nil, gateway_group: old_gateway_group
      end
