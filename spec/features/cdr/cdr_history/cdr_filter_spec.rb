...
RSpec.describe 'Cdrs index page filtering', js: true do
  ...
  describe 'filter by customer auth' do
    let(:filter_cdr_records) do
      within_filters do
        fill_in_tom_select 'Customer Auth', with: customer_auths[0].name, ajax: true
      end
    end
    ...
    it 'shows filtered records with customer auth' do
      ...
      within_filters do
        expect(page).to have_field_tom_select('Customer Auth', with: customer_auths[0].display_name)
      end
    end
  end
end
