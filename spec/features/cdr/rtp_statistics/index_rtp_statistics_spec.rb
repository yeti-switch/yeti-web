...
RSpec.describe 'RTP statistics index', type: :feature do
  include_context :login_as_admin
  let(:index_path) { rtp_statistics_path }
  ...
  describe 'filter' do
    ...
    let!(:second_record) { FactoryBot.create(:rtp_statistic) }
    ...
    context 'by RX_SSRC' do
      context 'when "Equals" option of filter is selected' do
        ...
        before { visit rtp_statistics_path(q: { rx_ssrc_equals: filter_value }) }
      ...
      context 'when "Hex" option of filter is selected' do
        ...
        before { visit rtp_statistics_path(q: { rx_ssrc_hex: hex_filter_value }) }
    ...
    context 'by TX_SSRC' do
      context 'when "Equals" option of filter is selected' do
        ...
        before { visit rtp_statistics_path(q: { tx_ssrc_equals: filter_value }) }
        ...
        it 'should render filtered records only' do
          ...
          expect(page).to have_table_row count: 1
          expect(page).to have_table_cell column: 'Id', text: record.id
          expect(page).to have_field 'Tx ssrc', with: filter_value
      ...
      context 'when "Hex" option of filter is selected' do
        ...
        before { visit rtp_statistics_path(q: { tx_ssrc_hex: hex_filter_value }) }
