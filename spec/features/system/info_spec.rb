  ...
  context 'when show_config_info = true' do
    ...
    it 'renders page' do
      ...
      within_panel 'Yeti-web config' do
        ...
        expect(page).to have_attribute_row('CUSTOMER')
        expect(page).to have_attribute_row('TOKEN LIFETIME', exact_text: YetiConfig.api.customer.token_lifetime.presence || 'Empty')
        expect(page).to have_attribute_row('CALL JWT LIFETIME', exact_text: YetiConfig.api.customer.call_jwt_lifetime.presence || 'Empty')
        expect(page).to have_attribute_row('OUTGOING CDR HIDE FIELDS', exact_text: YetiConfig.api.customer.outgoing_cdr_hide_fields.presence || 'Empty')
        expect(page).to have_attribute_row('OUTGOING STATISTICS USE CUSTOMER DURATION', exact_text: YetiConfig.api.customer.outgoing_statistics_use_customer_duration.presence || 'Empty')
        expect(page).to have_attribute_row('INCOMING CDR HIDE FIELDS', exact_text: YetiConfig.api.customer.incoming_cdr_hide_fields.presence || 'Empty')
        expect(page).to have_attribute_row('INCOMING STATISTICS USE VENDOR DURATION', exact_text: YetiConfig.api.customer.incoming_statistics_use_vendor_duration.presence || 'Empty')
