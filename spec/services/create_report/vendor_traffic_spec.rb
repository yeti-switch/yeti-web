...
RSpec.describe CreateReport::VendorTraffic do
  subject do
    described_class.call(service_params)
  end

  shared_examples :creates_report do
    it 'creates Report::VendorTraffic' do
      expect { subject }.to change { Report::VendorTraffic.count }.by(1)

      report = Report::VendorTraffic.last!
    ...
    it 'enqueues Worker::GenerateReportDataJob' do
      expect { subject }.to have_enqueued_job(Worker::GenerateReportDataJob).once.on_queue('report').with do |*args|
        report = Report::VendorTraffic.last!
        expect(args).to eq ['VendorTraffic', report.id]
      end
    end
  end

  shared_examples :does_not_create_report do
    it 'does not create Report::VendorTraffic' do
      expect { safe_subject }.to change { Report::VendorTraffic.count }.by(0)
    end
  ...
  let(:expected_report_attrs) do
    {
      ...
      vendor_id: service_params[:vendor].id,
