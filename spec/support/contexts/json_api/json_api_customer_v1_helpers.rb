...
RSpec.shared_context :json_api_customer_v1_helpers do |type: nil|
  ...
  let(:json_api_auth_expiration) { 1.minute.from_now }
  let(:json_api_auth_token) do
    if auth_config
      build_customer_token_from_config(auth_config, expiration: json_api_auth_expiration)
    else
      build_customer_token(api_access.id, expiration: json_api_auth_expiration)
    end
  end
