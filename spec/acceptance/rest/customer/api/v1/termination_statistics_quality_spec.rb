...
RSpec.resource 'Termination Statistics Quality', document: :customer_v1 do
  ...
  let(:type) { 'termination-statistics-quality' }
  include_context :customer_termination_statistics_quality_clickhouse_helpers
  let!(:account) { create(:account, contractor: customer) }

  get '/api/rest/customer/v1/termination-statistics-quality' do
    required_filters = %i[account-id from-time sampling]
    optional_filters = ClickhouseReport::TerminationStatisticQuality.filters.reject { |_, v| v.required }.keys
