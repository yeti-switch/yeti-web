...
RSpec.resource 'Origination Active Calls', document: :customer_v1 do
  ...
  let(:type) { 'origination-active-calls' }
  include_context :customer_origination_active_calls_clickhouse_helpers
  let!(:account) { create(:account, contractor: customer) }

  get '/api/rest/customer/v1/origination-active-calls' do
    required_filters = %i[account-id from-time]
    optional_filters = ClickhouseReport::OriginationActiveCalls.filters.reject { |_, v| v.required }.keys
