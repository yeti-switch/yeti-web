...
RSpec.resource 'Termination Statistics', document: :customer_v1 do
  ...
  let(:type) { 'termination-statistics' }
  include_context :customer_termination_statistics_clickhouse_helpers
  let!(:account) { create(:account, contractor: customer) }

  get '/api/rest/customer/v1/termination-statistics' do
    required_filters = %i[account-id from-time sampling]
    optional_filters = ClickhouseReport::TerminationStatistic.filters.reject { |_, v| v.required }.keys
