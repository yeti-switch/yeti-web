...
RSpec.resource 'Origination Statistics Quality', document: :customer_v1 do
  ...
  let(:type) { 'origination-statistics-quality' }
  include_context :customer_origination_statistics_quality_clickhouse_helpers
  let!(:account) { create(:account, contractor: customer) }

  get '/api/rest/customer/v1/origination-statistics-quality' do
    required_filters = %i[account-id from-time sampling]
    optional_filters = ClickhouseReport::OriginationStatisticQuality.filters.reject { |_, v| v.required }.keys
