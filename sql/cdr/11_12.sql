begin;
insert into sys.version(number,comment) values(12,'Customer reports');
...
CREATE TABLE reports.customer_traffic_report_data
(
  id bigserial PRIMARY KEY,
  ...
  CONSTRAINT customer_traffic_report_data_report_id_fkey FOREIGN KEY (report_id)
  REFERENCES reports.customer_traffic_report (id) MATCH SIMPLE
...
create index on reports.customer_traffic_report_data using BTREE (report_id);


CREATE OR REPLACE FUNCTION reports.customer_traffic_remove(i_report_id integer)
  RETURNS void AS
  $BODY$
DECLARE

BEGIN
    delete from reports.customer_traffic_report_data where report_id=i_report_id;
    delete from reports.customer_traffic_report where id=i_report_id;
END;
$BODY$
