begin;
insert into sys.version(number,comment) values(17,'Call length rounding');
...
insert into sys.call_duration_round_modes(id,name) VALUES(3,'Always UP');
insert into sys.call_duration_round_modes(id,name) VALUES(2,'Always DOWN');
insert into sys.call_duration_round_modes(id,name) VALUES(1,'Math rules(up if >=0.5)');

create table sys.config(
  id SMALLINT PRIMARY KEY,
  call_duration_round_mode_id SMALLINT not null REFERENCES sys.call_duration_round_modes(id) DEFAULT 1
);

insert into sys.config(id,call_duration_round_mode_id) values(1,1);


CREATE OR REPLACE FUNCTION switch.round(i_duration double precision)
  RETURNS integer AS
  $BODY$
DECLARE
    v_mode_id smallint;
