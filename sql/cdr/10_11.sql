begin;
insert into sys.version(number,comment) values(11,'Traffic amount statistic for accounts');
...
CREATE TABLE stats.traffic_customer_accounts(
  ...
  amount numeric,
  CONSTRAINT traffic_customer_accounts_pkey PRIMARY KEY (id)
);

-- DROP INDEX stats.traffic_customer_accounts_account_id_timestamp_idx;
CREATE UNIQUE INDEX traffic_customer_accounts_account_id_timestamp_idx
ON stats.traffic_customer_accounts USING btree(account_id, "timestamp");
...
(
  ...
  count bigint NOT NULL,
  amount numeric,
  CONSTRAINT traffic_vendor_accounts_pkey PRIMARY KEY (id)
);

-- DROP INDEX stats.traffic_vendor_accounts_account_id_timestamp_idx;

CREATE UNIQUE INDEX traffic_vendor_accounts_account_id_timestamp_idx
  ON stats.traffic_vendor_accounts  USING btree  (account_id, "timestamp");

alter table stats.traffic_vendor_accounts add profit numeric;
alter table stats.traffic_customer_accounts add profit numeric;
...
if i_time_connect is not null then
    ...
    v_cdr.duration:=(i_time_end-i_time_connect)::integer;
    v_nozerolen:=true;
    v_cdr.success=true;
else
