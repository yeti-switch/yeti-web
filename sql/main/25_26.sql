begin;
insert into sys.version(number,comment) values(26,'ActiveCalls autorefresh,Invoices,Static routes');

alter TABLE billing.accounts add customer_invoice_template_id integer;
alter TABLE billing.accounts add vendor_invoice_template_id integer;

ALTER TABLE billing.accounts add next_customer_invoice_at timestamp with time zone;
ALTER TABLE billing.accounts add next_vendor_invoice_at timestamp with time zone;
ALTER TABLE billing.accounts RENAME invoice_period_id TO customer_invoice_period_id;
ALTER TABLE billing.accounts add vendor_invoice_period_id smallint REFERENCES billing.invoice_periods(id);

CREATE INDEX ON class4.routing_plan_static_routes USING gist (prefix_range(prefix),vendor_id,routing_plan_id);
...
CREATE OR REPLACE FUNCTION switch5.route(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  ...
  RETURNS SETOF switch5.callprofile40_ty AS
  $BODY$
DECLARE
...
CREATE OR REPLACE FUNCTION switch5.route_debug(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  i_remote_port integer,
...
CREATE OR REPLACE FUNCTION switch5.route_release(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  i_remote_port integer,
