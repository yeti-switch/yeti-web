begin;
insert into sys.version(number,comment) values(48,'Username for thinq');

ALTER TABLE class4.lnp_databases add thinq_username varchar;

ALTER TABLE class4.routing_plan_lnp_rules
  add req_dst_rewrite_rule varchar,
  add req_dst_rewrite_result varchar;

DROP FUNCTION switch8.load_lnp_databases();

CREATE OR REPLACE FUNCTION switch8.load_lnp_databases()
  RETURNS TABLE(o_id smallint, o_name character varying, o_driver_id smallint, o_host character varying, o_port integer, o_thinq_username varchar, o_thinq_token character varying) AS
  $BODY$
BEGIN
    RETURN
...
create unique INDEX ON class4.lnp_cache using btree (dst, database_id);
alter table class4.lnp_cache drop constraint lnp_cache_dst_key;
...
DROP FUNCTION switch8.lnp_resolve(smallint, integer, character varying);
