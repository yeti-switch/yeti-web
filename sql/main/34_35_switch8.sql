begin;
insert into sys.version(number,comment) values(35,'Initial LNP configuration');
...
VALUES (8003,0,true,true,503,'No response from LNP DB',null,null,false,false,true, false);
...
insert into  sys.lnp_database_drivers(name) VALUES ('UDP SIP 301/302 redirect');
...
CREATE TABLE class4.lnp_databases(
  ...
  port integer,
  driver_id smallint not null references sys.lnp_database_drivers(id),
...
CREATE TABLE class4.lnp_cache(
  ...
  database_id smallint references class4.lnp_databases(id),
  data varchar
);
...
CREATE TABLE class4.routing_plan_lnp_rules(
  ...
  database_id smallint not null references class4.lnp_databases(id),
  created_at TIMESTAMPTZ
);

set search_path TO public;
CREATE INDEX ON class4.routing_plan_lnp_rules USING gist(prefix_range(dst_prefix),routing_plan_id);

alter table class4.routing_plans drop column more_specific_per_vendor;
alter table class4.routing_plans add use_lnp boolean NOT NULL DEFAULT false;

alter table class4.gateways add send_lnp_information boolean not null default false;

alter table sys.guiconfig add drop_call_if_lnp_fail boolean not null default false;
ALTER TABLE gui.admin_users add stateful_filters boolean not null default false;

ALTER TABLE class4.dialpeers ALTER COLUMN initial_interval SET DEFAULT 1;
ALTER TABLE class4.dialpeers ALTER COLUMN next_interval SET DEFAULT 1;
ALTER TABLE class4.destinations ALTER COLUMN next_interval SET DEFAULT 1;
ALTER TABLE class4.destinations ALTER COLUMN initial_interval SET DEFAULT 1;

ALTER TABLE class4.dialpeers ALTER COLUMN initial_rate DROP DEFAULT ;
ALTER TABLE class4.destinations ALTER COLUMN initial_rate DROP DEFAULT ;
ALTER TABLE class4.destinations ALTER COLUMN next_rate DROP DEFAULT ;
ALTER TABLE class4.dialpeers ALTER COLUMN connect_fee set DEFAULT 0.0;
update class4.dialpeers set acd_limit=0 where acd_limit is null;
ALTER TABLE class4.dialpeers ALTER COLUMN acd_limit set NOT NULL ;
ALTER TABLE class4.dialpeers ALTER COLUMN asr_limit set DEFAULT 0.0;
...
CREATE  INDEX  ON gui.sessions  (updated_at);
...
-- Started on 2015-03-28 20:23:32 EET
...
--
-- TOC entry 1926 (class 1247 OID 23752)
...
--
-- TOC entry 1929 (class 1247 OID 23755)
...
--
-- TOC entry 1932 (class 1247 OID 23758)
...
--
-- TOC entry 1935 (class 1247 OID 23761)
...
--
-- TOC entry 1938 (class 1247 OID 23764)
...
--
-- TOC entry 1966 (class 1247 OID 23878)
...
--
-- TOC entry 1041 (class 1255 OID 23765)
...
--
-- TOC entry 1042 (class 1255 OID 23766)
...
--
-- TOC entry 1052 (class 1255 OID 23885)
...
--
-- TOC entry 1030 (class 1255 OID 23768)
...
--
-- TOC entry 1055 (class 1255 OID 24040)
-- Name: lnp_resolve(smallint, integer, character varying); Type: FUNCTION; Schema: switch8; Owner: -
--

CREATE FUNCTION lnp_resolve(i_database_id smallint, i_timeout integer, i_dst character varying) RETURNS character varying
LANGUAGE plpgsql COST 10
AS $$
BEGIN
    return yeti_ext.lnp_resolve(i_database_id, i_timeout, i_dst);
END;
$$;


--
-- TOC entry 1031 (class 1255 OID 23769)
...
--
-- TOC entry 1025 (class 1255 OID 23770)
...
--
-- TOC entry 1026 (class 1255 OID 23771)
...
--
-- TOC entry 1027 (class 1255 OID 23772)
...
--
-- TOC entry 1028 (class 1255 OID 23773)
...
--
-- TOC entry 1032 (class 1255 OID 23774)
...
--
-- TOC entry 1033 (class 1255 OID 23775)
...
--
-- TOC entry 803 (class 1255 OID 23776)
...
--
-- TOC entry 801 (class 1255 OID 23777)
...
--
-- TOC entry 802 (class 1255 OID 23778)
...
--
-- TOC entry 804 (class 1255 OID 23779)
...
--
-- TOC entry 809 (class 1255 OID 23780)
...
--
-- TOC entry 449 (class 1259 OID 23781)
...
--
-- TOC entry 810 (class 1255 OID 23788)
...
--
-- TOC entry 811 (class 1255 OID 23789)
...
--
-- TOC entry 812 (class 1255 OID 23790)
...
--
-- TOC entry 1050 (class 1255 OID 23879)
...
--
-- TOC entry 813 (class 1255 OID 23792)
...
--
-- TOC entry 814 (class 1255 OID 23793)
...
--
-- TOC entry 1051 (class 1255 OID 23882)
...
--
-- TOC entry 1047 (class 1255 OID 23890)
...
--
-- TOC entry 1048 (class 1255 OID 23891)
...
--
-- TOC entry 1053 (class 1255 OID 23883)
...
--    i_profile.next_hop_for_replies:=v_dst_gw.term_next_hop_for_replies;
    ...
    if i_vendor_gw.send_lnp_information and i_profile.lrn is not null then
        if i_profile.lrn=i_profile.dst_prefix_routing then -- number not ported, but request was successf we musr add ;npdi=yes;
            ...
            i_profile.lrn=nullif(v_ret.dst_prefix_routing,v_ret.lrn); -- clear lnr field if number not ported;
...
--
-- TOC entry 1049 (class 1255 OID 23892)
...
--
-- TOC entry 874 (class 1255 OID 23894)
...
--
-- TOC entry 1043 (class 1255 OID 23803)
...
--
-- TOC entry 842 (class 1255 OID 23880)
...
--- Blacklist processing
    ...
    if v_rp.use_lnp then
        select into v_ret.lnp_database_id database_id
            from class4.class4.routing_plan_lnp_rules rules
            WHERE prefix_range(rules.dst_prefix)@>prefix_range(v_ret.dst_prefix_routing) and rules.rateplan_id=v_rp.id
        ...
        if found then
            ...
            if found then
                /*dbg{*/
                v_end:=clock_timestamp();
                RAISE NOTICE '% ms -> LNP. Data found in cache. Lets use it',EXTRACT(MILLISECOND from v_end-v_start);
                /*}dbg*/
                ...
                if v_ret.lrn is null then -- fail
                    ...
                    /*dbg{*/
                    v_end:=clock_timestamp();
                    RAISE NOTICE '% ms -> LNP. Will use old number for routing',EXTRACT(MILLISECOND from v_end-v_start);
                    /*}dbg*/
                    v_routing_key=v_ret.lrn;
                end if;
            end if;
...
--
-- TOC entry 1054 (class 1255 OID 23886)
...
--
-- TOC entry 1046 (class 1255 OID 23888)
...
--
-- TOC entry 1044 (class 1255 OID 23811)
...
--
-- TOC entry 1045 (class 1255 OID 23812)
...
--
-- TOC entry 450 (class 1259 OID 23813)
...
--
-- TOC entry 451 (class 1259 OID 23815)
...
--
-- TOC entry 452 (class 1259 OID 23821)
...
--
-- TOC entry 453 (class 1259 OID 23823)
...
--
-- TOC entry 454 (class 1259 OID 23825)
...
--
-- TOC entry 455 (class 1259 OID 23831)
...
--
-- TOC entry 456 (class 1259 OID 23833)
...
--
-- TOC entry 457 (class 1259 OID 23841)
...
--
-- TOC entry 458 (class 1259 OID 23847)
...
--
-- TOC entry 3387 (class 2604 OID 23849)
...
--
-- TOC entry 3389 (class 2604 OID 23850)
-- Name: id; Type: DEFAULT; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3392 (class 2604 OID 23851)
-- Name: id; Type: DEFAULT; Schema: switch8; Owner: -
--
...
-- Dependencies: 450
-- Name: events_id_seq; Type: SEQUENCE SET; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3521 (class 0 OID 23815)
-- Dependencies: 451
-- Data for Name: resource_action; Type: TABLE DATA; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3519 (class 0 OID 23781)
-- Dependencies: 449
-- Data for Name: resource_type; Type: TABLE DATA; Schema: switch8; Owner: -
--
...
-- Dependencies: 452
-- Name: resource_type_id_seq; Type: SEQUENCE SET; Schema: switch8; Owner: -
--
...
-- Dependencies: 453
-- Name: switch_in_interface_id_seq; Type: SEQUENCE SET; Schema: switch8; Owner: -
--
...
-- Dependencies: 455
-- Name: switch_interface_id_seq; Type: SEQUENCE SET; Schema: switch8; Owner: -
--

SELECT pg_catalog.setval('switch_interface_id_seq', 887, true);


--
-- TOC entry 3526 (class 0 OID 23833)
-- Dependencies: 456
-- Data for Name: switch_interface_in; Type: TABLE DATA; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3524 (class 0 OID 23825)
-- Dependencies: 454
-- Data for Name: switch_interface_out; Type: TABLE DATA; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3527 (class 0 OID 23841)
-- Dependencies: 457
-- Data for Name: trusted_headers; Type: TABLE DATA; Schema: switch8; Owner: -
--
...
-- Dependencies: 458
-- Name: trusted_headers_id_seq; Type: SEQUENCE SET; Schema: switch8; Owner: -
--
...
--
-- TOC entry 3398 (class 2606 OID 23854)
...
--
-- TOC entry 3400 (class 2606 OID 23856)
...
--
-- TOC entry 3394 (class 2606 OID 23858)
...
--
-- TOC entry 3396 (class 2606 OID 23860)
...
--
-- TOC entry 3406 (class 2606 OID 23862)
...
--
-- TOC entry 3408 (class 2606 OID 23864)
...
--
-- TOC entry 3402 (class 2606 OID 23866)
...
--
-- TOC entry 3404 (class 2606 OID 23868)
...
--
-- TOC entry 3410 (class 2606 OID 23870)
...
--
-- TOC entry 3411 (class 2606 OID 23871)
...
-- Completed on 2015-03-28 20:23:38 EET
