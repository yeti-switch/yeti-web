begin;
insert into sys.version(number,comment) values(14,'Fix importing-tables: importing_customers_auth, ');

-- FIX for termination gateway statistic
...
BEGIN
        ...
        IF NOT FOUND THEN
                ...
                BEGIN
                        ...
                        VALUES(i_term_gw_id,1,v_success,v_success,v_duration,v_duration::real/1,v_success::real/1);
...
ADD dst_blacklist_id integer,
ADD dst_blacklist_name character varying,
ADD src_blacklist_id integer,
ADD src_blacklist_name character varying;

ALTER TABLE data_import.import_gateways RENAME COLUMN term_auth_enabled TO auth_enabled;
ALTER TABLE data_import.import_gateways RENAME COLUMN term_auth_user TO auth_user;
ALTER TABLE data_import.import_gateways RENAME COLUMN term_auth_pwd TO auth_password;

ALTER TABLE data_import.import_gateways
ADD codec_group_id integer,
ADD codec_group_name character varying,
ADD single_codec_in_200ok boolean,
ADD ringing_timeout integer,
ADD symmetric_rtp_nonstop boolean,
ADD symmetric_rtp_ignore_rtcp boolean,
ADD resolve_ruri boolean,
ADD force_dtmf_relay boolean,
ADD relay_options boolean,
ADD rtp_ping boolean,
ADD filter_noaudio_streams boolean,
ADD relay_reinvite boolean,
ADD sdp_c_location_id integer,
ADD sdp_c_location_name character varying,
ADD auth_from_user character varying,
ADD auth_from_domain character varying,
ADD relay_hold boolean,
ADD rtp_timeout integer,
ADD relay_prack boolean,
ADD rtp_relay_timestamp_aligning boolean,
ADD allow_1xx_without_to_tag boolean,
ADD sip_timer_b integer,
ADD dns_srv_failover_timer integer,
ADD rtp_force_relay_cn boolean,
ADD sensor_id smallint,
ADD sensor_name character varying,
ADD sensor_level_id smallint,
ADD sensor_level_name character varying,
ADD dtmf_send_mode_id smallint,
ADD dtmf_send_mode_name varchar,
ADD dtmf_receive_mode_id smallint,
ADD dtmf_receive_mode_name varchar,
DROP COLUMN message_filter_type_name,
DROP COLUMN message_filter_type_id,
DROP COLUMN message_filter_list,
DROP COLUMN sdp_filter_list,
DROP COLUMN sdp_filter_type_id,
DROP COLUMN sdp_filter_type_name;


-- New relations for Gateway
...
insert into class4.dtmf_send_modes values(0,'Disable sending');
insert into class4.dtmf_send_modes values(1,'RFC 2833');
insert into class4.dtmf_send_modes values(2,'SIP INFO application/dtmf-relay');
insert into class4.dtmf_send_modes values(4,'SIP INFO application/dtmf');

insert into class4.dtmf_receive_modes values(1,'RFC 2833');
insert into class4.dtmf_receive_modes values(2,'SIP INFO application/dtmf-relay OR application/dtmf');
insert into class4.dtmf_receive_modes values(3,'RFC 2833 OR SIP INFO');

alter table class4.gateways add dtmf_send_mode_id smallint references class4.dtmf_send_modes(id) not null default 1;
alter table class4.gateways add dtmf_receive_mode_id smallint references class4.dtmf_receive_modes(id) not null default 1;
...
/*
drop table reports.cdr_custom_report;
drop table reports.cdr_custom_report_data;
drop table reports.cdr_interval_report;
drop table reports.cdr_interval_report_aggrerator;
drop table reports.cdr_interval_report_data;
drop table reports.report_vendors;
drop table reports.report_vendors_data;
*/

alter table gui.versions add object_changes text;

-- First step to migration to new routing schema
create table class4.routing_plans(
id serial primary key,
name varchar not null unique,
sorting_id integer not null default 1 REFERENCES class4.sortings(id),
more_specific_per_vendor boolean not null default true,
rate_delta_max numeric not null default 0
);

insert into class4.routing_plans select * from class4.routing_groups;
-- fix sequences
select setval('class4.routing_plans_id_seq'::regclass,(select max(id) from class4.routing_plans),true);

create table class4.routing_plan_groups(
id serial primary key,
routing_plan_id integer not null references  class4.routing_plans(id),
routing_group_id integer not null references  class4.routing_groups(id),
UNIQUE(routing_plan_id,routing_group_id)
);
insert into  class4.routing_plan_groups(routing_plan_id,routing_group_id) select id,id from class4.routing_groups;

alter table class4.customers_auth add routing_plan_id integer references class4.routing_plans(id);
update  class4.customers_auth set routing_plan_id=routing_group_id;
alter table class4.customers_auth alter column routing_plan_id set not null;

alter table sys.guiconfig add cdr_archive_delay integer not null default 4;
alter table sys.guiconfig add cdr_remove_delay integer not null default 120;

ALTER TABLE class4.customers_auth add allow_receive_rate_limit boolean not null default false;
ALTER TABLE data_import.import_customers_auth add allow_receive_rate_limit boolean not null default false;
ALTER TABLE class4.customers_auth add send_billing_information boolean not null default false;
ALTER TABLE data_import.import_customers_auth add send_billing_information boolean not null default false;

ALTER TABLE data_import.import_customers_auth
ADD routing_plan_id integer,
ADD routing_plan_name varchar;
...
-- Started on 2015-01-27 15:08:11 EET
...
--
-- TOC entry 1495 (class 1247 OID 20462)
...
--
-- TOC entry 1216 (class 1247 OID 20656)
...
--
-- TOC entry 1219 (class 1247 OID 20698)
...
--
-- TOC entry 731 (class 1255 OID 20463)
...
--
-- TOC entry 732 (class 1255 OID 20464)
...
--
-- TOC entry 733 (class 1255 OID 20465)
...
--
-- TOC entry 734 (class 1255 OID 20466)
...
--
-- TOC entry 735 (class 1255 OID 20467)
...
--
-- TOC entry 736 (class 1255 OID 20468)
...
--
-- TOC entry 737 (class 1255 OID 20469)
...
--
-- TOC entry 738 (class 1255 OID 20470)
...
--
-- TOC entry 739 (class 1255 OID 20471)
...
--
-- TOC entry 730 (class 1255 OID 20472)
...
--
-- TOC entry 740 (class 1255 OID 20473)
...
--
-- TOC entry 741 (class 1255 OID 20474)
...
--
-- TOC entry 742 (class 1255 OID 20475)
...
--
-- TOC entry 756 (class 1255 OID 20476)
...
--
-- TOC entry 351 (class 1259 OID 20477)
...
--
-- TOC entry 757 (class 1255 OID 20484)
...
--
-- TOC entry 758 (class 1255 OID 20485)
...
--
-- TOC entry 759 (class 1255 OID 20486)
...
--
-- TOC entry 780 (class 1255 OID 20699)
...
--
-- TOC entry 760 (class 1255 OID 20488)
...
--
-- TOC entry 761 (class 1255 OID 20489)
...
--
-- TOC entry 781 (class 1255 OID 20700)
...
--
-- TOC entry 786 (class 1255 OID 20712)
...
--
-- TOC entry 787 (class 1255 OID 20713)
...
--
-- TOC entry 785 (class 1255 OID 20703)
...
--
-- TOC entry 788 (class 1255 OID 20714)
...
--
-- TOC entry 789 (class 1255 OID 20716)
...
--
-- TOC entry 696 (class 1255 OID 20499)
...
--
-- TOC entry 782 (class 1255 OID 20701)
...
--
-- TOC entry 783 (class 1255 OID 20708)
...
--
-- TOC entry 784 (class 1255 OID 20710)
...
--
-- TOC entry 697 (class 1255 OID 20506)
...
--
-- TOC entry 698 (class 1255 OID 20507)
...
--
-- TOC entry 352 (class 1259 OID 20508)
...
--
-- TOC entry 353 (class 1259 OID 20510)
...
--
-- TOC entry 354 (class 1259 OID 20516)
...
--
-- TOC entry 355 (class 1259 OID 20518)
...
--
-- TOC entry 356 (class 1259 OID 20520)
...
--
-- TOC entry 357 (class 1259 OID 20526)
...
--
-- TOC entry 358 (class 1259 OID 20528)
...
--
-- TOC entry 359 (class 1259 OID 20536)
...
--
-- TOC entry 360 (class 1259 OID 20542)
...
--
-- TOC entry 2937 (class 2604 OID 20544)
...
--
-- TOC entry 2939 (class 2604 OID 20545)
-- Name: id; Type: DEFAULT; Schema: switch4; Owner: -
--
...
--
-- TOC entry 2942 (class 2604 OID 20546)
-- Name: id; Type: DEFAULT; Schema: switch4; Owner: -
--
...
-- Dependencies: 352
-- Name: events_id_seq; Type: SEQUENCE SET; Schema: switch4; Owner: -
--
...
--
-- TOC entry 3071 (class 0 OID 20510)
-- Dependencies: 353
-- Data for Name: resource_action; Type: TABLE DATA; Schema: switch4; Owner: -
--
...
--
-- TOC entry 3069 (class 0 OID 20477)
-- Dependencies: 351
-- Data for Name: resource_type; Type: TABLE DATA; Schema: switch4; Owner: -
--
...
-- Dependencies: 354
-- Name: resource_type_id_seq; Type: SEQUENCE SET; Schema: switch4; Owner: -
--
...
-- Dependencies: 355
-- Name: switch_in_interface_id_seq; Type: SEQUENCE SET; Schema: switch4; Owner: -
--
...
-- Dependencies: 357
-- Name: switch_interface_id_seq; Type: SEQUENCE SET; Schema: switch4; Owner: -
--
...
--
-- TOC entry 3076 (class 0 OID 20528)
-- Dependencies: 358
-- Data for Name: switch_interface_in; Type: TABLE DATA; Schema: switch4; Owner: -
--
...
--
-- TOC entry 3074 (class 0 OID 20520)
-- Dependencies: 356
-- Data for Name: switch_interface_out; Type: TABLE DATA; Schema: switch4; Owner: -
--
...
--
-- TOC entry 3077 (class 0 OID 20536)
-- Dependencies: 359
-- Data for Name: trusted_headers; Type: TABLE DATA; Schema: switch4; Owner: -
--
...
-- Dependencies: 360
-- Name: trusted_headers_id_seq; Type: SEQUENCE SET; Schema: switch4; Owner: -
--
...
--
-- TOC entry 2948 (class 2606 OID 20548)
...
--
-- TOC entry 2950 (class 2606 OID 20550)
...
--
-- TOC entry 2944 (class 2606 OID 20552)
...
--
-- TOC entry 2946 (class 2606 OID 20554)
...
--
-- TOC entry 2956 (class 2606 OID 20556)
...
--
-- TOC entry 2958 (class 2606 OID 20558)
...
--
-- TOC entry 2952 (class 2606 OID 20560)
...
--
-- TOC entry 2954 (class 2606 OID 20562)
...
--
-- TOC entry 2960 (class 2606 OID 20564)
...
--
-- TOC entry 2961 (class 2606 OID 20565)
...
-- Completed on 2015-01-27 15:08:14 EET

--
-- PostgreSQL database dump complete
--

insert into sys.jobs(type) values ('CdrArchiving');
