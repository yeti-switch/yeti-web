begin;
insert into sys.version(number,comment) values(38,'LNP code');
...
BEGIN
    begin
        insert into class4.lnp_cache(dst,lrn,created_at,updated_at,expires_at,database_id,data) values( i_dst, i_lrn, now(),now(),now()+'1 week'::interval,i_database_id,i_data);
    Exception
        when unique_violation then
            update class4.lnp_cache set lrn=i_lrn, updated_at=now(), expires_at=(now()+'1 week'::interval), data=i_data, database_id=i_database_id WHERE dst=i_dst;
    end;
END;
$BODY$
