begin;
insert into sys.version(number,comment) values(32,'Call debug');

delete from notifications.contacts where admin_user_id is not null;
insert into notifications.contacts(admin_user_id,email,created_at,updated_at) select id,email,now(),now() from gui.admin_users;

ALTER TABLE sys.guiconfig add max_call_duration integer not null default 7200;
ALTER TABLE sys.guiconfig add random_disconnect_enable boolean not null default false;
ALTER TABLE sys.guiconfig add random_disconnect_length integer not null default 7000;
...
-- Function: switch7.route(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer)

DROP FUNCTION switch7.route(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer);

CREATE OR REPLACE FUNCTION switch7.route(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  ...
  RETURNS SETOF switch7.callprofile42_ty AS
  $BODY$
DECLARE
...
-- Function: switch7.process_dp(switch7.callprofile42_ty, class4.destinations, class4.dialpeers, billing.accounts, class4.gateways, billing.accounts, integer, boolean, integer)

DROP FUNCTION switch7.process_dp(switch7.callprofile42_ty, class4.destinations, class4.dialpeers, billing.accounts, class4.gateways, billing.accounts, integer, boolean);

CREATE OR REPLACE FUNCTION switch7.process_dp(
  i_profile switch7.callprofile42_ty,
...
-- Function: switch7.process_gw(switch7.callprofile42_ty, class4.destinations, class4.dialpeers, billing.accounts, class4.gateways, billing.accounts, class4.gateways, boolean, integer)

DROP FUNCTION switch7.process_gw(switch7.callprofile42_ty, class4.destinations, class4.dialpeers, billing.accounts, class4.gateways, billing.accounts, class4.gateways, boolean);

CREATE OR REPLACE FUNCTION switch7.process_gw(
  i_profile switch7.callprofile42_ty,
  i_destination class4.destinations,
  i_dp class4.dialpeers,
  i_customer_acc billing.accounts,
  ...
  i_send_billing_information boolean,
  i_max_call_length integer)
  RETURNS switch7.callprofile42_ty AS
  $BODY$
DECLARE
i integer;
v_customer_allowtime real;
...
SELECT * from switch7.preprocess_all();
