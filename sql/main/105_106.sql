begin;
insert into sys.version(number,comment) values(106,'Move blacklist to numberlist');

ALTER TABLE class4.blacklists RENAME TO numberlists;
alter table class4.blacklist_items rename to numberlist_items;
alter table class4.blacklist_modes rename to numberlist_modes;
...
insert into class4.numberlist_actions(id,name) VALUES (1,'Reject call');
insert into class4.numberlist_actions(id,name) VALUES (2,'Allow call');

alter table class4.numberlists add column default_action_id SMALLINT not null REFERENCES class4.numberlist_actions(id) default 1;
alter table class4.numberlist_items add column action_id smallint REFERENCES class4.numberlist_actions(id);
alter table class4.numberlist_items rename column blacklist_id to numberlist_id;

alter table class4.customers_auth rename column  src_blacklist_id to src_numberlist_id;
alter table class4.customers_auth rename column  dst_blacklist_id to dst_numberlist_id;

set search_path to switch12;

CREATE OR REPLACE FUNCTION switch12.route(
  i_node_id integer,
  i_pop_id integer,
  ...
  */
  ...
  if v_customer_auth.src_numberlist_id is not null then
    ...
    CASE v_numberlist.mode_id
      when 1 then -- strict match
      select into v_numberlist_item * from class4.numberlist_items ni
      where ni.numberlist_id=v_customer_auth.dst_numberlist_id and ni.key=v_ret.src_prefix_out limit 1;
      when 2 then -- prefix match
      select into v_numberlist_item * from class4.numberlist_items ni
      where ni.numberlist_id=v_customer_auth.dst_numberlist_id and prefix_range(ni.key)@>prefix_range(v_ret.src_prefix_out)
      order by length(ni.key) desc limit 1;
    end case;
    /*dbg{*/
    v_end:=clock_timestamp();
