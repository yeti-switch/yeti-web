begin;
insert into sys.version(number,comment) values(29,'Notifications');


ALTER TABLE class4.dialpeers add force_hit_rate float;
ALTER TABLE data_import.import_dialpeers add force_hit_rate float;
...
INSERT INTO class4.disconnect_code(id,namespace_id,stop_hunting,pass_reason_to_originator,code,reason,rewrited_code,rewrited_reason,success,successnozerolen,store_cdr,silently_drop)
  VALUES (8000,0,true,true,403,'Not enough customer balance',null,null,false,false,true, false);
INSERT INTO class4.disconnect_code(id,namespace_id,stop_hunting,pass_reason_to_originator,code,reason,rewrited_code,rewrited_reason,success,successnozerolen,store_cdr,silently_drop)
  VALUES (8001,0,true,true,403,'Destination number blacklisted',null,null,false,false,true,false);
INSERT INTO class4.disconnect_code(id,namespace_id,stop_hunting,pass_reason_to_originator,code,reason,rewrited_code,rewrited_reason,success,successnozerolen,store_cdr,silently_drop)
  VALUES (8002,0,true,true,403,'Source number blacklisted',null,null,false,false,true,false);


ALTER TABLE billing.accounts DROP COLUMN autogenerate_vendor_invoices ;
ALTER TABLE billing.accounts DROP COLUMN autogenerate_customer_invoices ;

-- Function: switch6.route(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer)

-- DROP FUNCTION switch6.route(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer);

CREATE OR REPLACE FUNCTION switch6.route(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  ...
  RETURNS SETOF switch6.callprofile41_ty AS
  $BODY$
DECLARE
...
-- Function: switch6.route_release(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer)

-- DROP FUNCTION switch6.route_release(integer, integer, inet, integer, inet, integer, character varying, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, integer, character varying, character varying, character varying, character varying, inet, integer);

CREATE OR REPLACE FUNCTION switch6.route_release(
  i_node_id integer,
  i_pop_id integer,
  i_remote_ip inet,
  i_remote_port integer,
...
CREATE TABLE class4.number_lists(
  id smallserial primary key,
  name varchar not null unique
);

CREATE TABLE class4.number_list_items(
  id serial primary key,
  list_id smallint not null references class4.number_lists(id),
...
create table notifications.notifications(
  id serial PRIMARY KEY,
  name varchar not null unique,
  template_id integer not null REFERENCES notifications.notification_templates(id),
  account_id INTEGER REFERENCES billing.accounts(id) -- related to account notifications
);

CREATE TABLE notifications.notification_subscriptions (
  id SERIAL PRIMARY KEY,
  contact_id integer not null REFERENCES notifications.contacts(id),
  notification_id INTEGER NOT NULL REFERENCES notifications.notifications(id),
  created_at TIMESTAMPTZ,
  updated_at TIMESTAMPTZ
);

CREATE UNIQUE INDEX on notifications.notification_subscriptions USING BTREE (contact_id, notification_id);
