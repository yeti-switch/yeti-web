class DestinationCdo < ActiveRecord::Migration[7.2]
  def up
    execute %q{
      alter table sys.config add cdo boolean not null default false;
      alter table cdr.cdr add cdo smallint;
      alter type switch.dynamic_cdr_data_ty add attribute destination_cdo smallint;

      DROP FUNCTION if exists sys.cdr_export_data(i_tbname character varying);
      DROP FUNCTION if exists sys.cdr_export_data(i_tbname character varying, i_dir character varying);
...
$$;
  ...
  def down
    execute %q{
      alter table sys.config drop column cdo;
      alter type switch.dynamic_cdr_data_ty drop attribute destination_cdo;
      alter table cdr.cdr drop column cdo;
...
CREATE OR REPLACE FUNCTION sys.cdr_export_data(i_tbname character varying) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION sys.cdr_export_data(i_tbname character varying, i_dir character varying) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_file varchar;
