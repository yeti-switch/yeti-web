class CdrCustomerGeoip < ActiveRecord::Migration[7.2]
  def up
    execute %q{
      alter table cdr.cdr
        add auth_orig_lat real,
        add auth_orig_lon real;

      alter type switch.lega_request_headers_ty
        add attribute x_orig_lat real,
        add attribute x_orig_lon real;
...
$$;
      ...
      alter table cdr.cdr
        drop column auth_orig_lat,
        drop column auth_orig_lon;

      alter type switch.lega_request_headers_ty
        drop attribute x_orig_lat,
        drop attribute x_orig_lon;
