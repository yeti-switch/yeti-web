class FixCdrReason < ActiveRecord::Migration[7.0]

  def up
    execute %q{

    alter table cdr.cdr rename column lega_reason to lega_q850_text;
    alter table cdr.cdr rename column legb_reason to legb_q850_text;
    alter table cdr.cdr
      add lega_q850_cause smallint,
      add lega_q850_params varchar,
      add legb_q850_cause smallint,
      add legb_q850_params varchar;
    ...
    alter type switch.lega_headers_ty drop attribute reason;
    alter type switch.lega_headers_ty add attribute reason switch.reason_ty;

    alter type switch.legb_headers_ty drop attribute reason;
    alter type switch.legb_headers_ty add attribute reason switch.reason_ty;

CREATE OR REPLACE FUNCTION switch.writecdr(
i_is_master boolean, i_node_id integer, i_pop_id integer, i_routing_attempt integer, i_is_last_cdr boolean,
i_lega_transport_protocol_id smallint, i_lega_local_ip character varying, i_lega_local_port integer,
i_lega_remote_ip character varying, i_lega_remote_port integer, i_legb_transport_protocol_id smallint,
i_legb_local_ip character varying, i_legb_local_port integer, i_legb_remote_ip character varying,
i_legb_remote_port integer, i_legb_ruri character varying, i_legb_outbound_proxy character varying,
i_time_data json, i_early_media_present boolean, i_legb_disconnect_code integer,
i_legb_disconnect_reason character varying, i_disconnect_initiator integer,
i_internal_disconnect_code integer, i_internal_disconnect_reason character varying,
i_lega_disconnect_code integer, i_lega_disconnect_reason character varying,
i_orig_call_id character varying, i_term_call_id character varying,
i_local_tag character varying, i_legb_local_tag character varying,
i_msg_logger_path character varying, i_dump_level_id smallint,
i_audio_recorded boolean, i_rtp_stats_data json, i_rtp_statistics json, i_global_tag character varying,
i_resources character varying, i_active_resources json, i_failed_resource_type_id smallint,
i_failed_resource_id bigint, i_dtmf_events json, i_versions json, i_is_redirected boolean,
i_dynamic json,
i_lega_headers json,
i_legb_headers json,
i_lega_identity json) RETURNS integer
...
$$;
  ...
  def down
    ...
    alter table cdr.cdr rename column lega_q850_text to lega_reason;
    alter table cdr.cdr rename column legb_q850_text to legb_reason;

    alter table cdr.cdr
      drop column lega_q850_cause,
      drop column lega_q850_params,
      drop column legb_q850_cause,
      drop column legb_q850_params;
...
$$;
    ...
    alter type switch.lega_headers_ty drop attribute reason;
    alter type switch.lega_headers_ty add attribute reason varchar;

    alter type switch.legb_headers_ty drop attribute reason;
    alter type switch.legb_headers_ty add attribute reason varchar;

    drop type switch.reason_ty;
