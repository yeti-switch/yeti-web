class FixRxStreamLocalPort < ActiveRecord::Migration[7.2]

  def up
    execute %q{
ALTER TABLE rtp_statistics.rx_streams RENAME COLUMN time_end TO stream_time_end;
ALTER TABLE rtp_statistics.tx_streams RENAME COLUMN time_end TO stream_time_end;
ALTER TABLE rtp_statistics.rx_streams ADD COLUMN stream_time_start timestamp with time zone;
ALTER TABLE rtp_statistics.tx_streams ADD COLUMN stream_time_start timestamp with time zone;
    }

    execute %q{
DROP FUNCTION IF EXISTS switch.write_rtp_statistics(json, integer, integer, bigint, bigint, bigint, bigint, character varying, character varying);
CREATE OR REPLACE FUNCTION switch.write_rtp_statistics(i_data json, i_pop_id integer, i_node_id integer, i_lega_gateway_id bigint, i_lega_gateway_external_id bigint, i_legb_gateway_id bigint, i_legb_gateway_external_id bigint, i_lega_local_tag character varying, i_legb_local_tag character varying, i_time_start timestamp with time zone DEFAULT NULL) RETURNS void
...
$$;
    ...
    execute %q{
DROP FUNCTION IF EXISTS switch.write_rtp_statistics(json, integer, integer, bigint, bigint, bigint, bigint, character varying, character varying, timestamp with time zone);
...
$$;
    ...
    execute %q{
ALTER TABLE rtp_statistics.rx_streams DROP COLUMN stream_time_start;
ALTER TABLE rtp_statistics.tx_streams DROP COLUMN stream_time_start;
ALTER TABLE rtp_statistics.rx_streams RENAME COLUMN stream_time_end TO time_end;
ALTER TABLE rtp_statistics.tx_streams RENAME COLUMN stream_time_end TO time_end;
    }
  end
