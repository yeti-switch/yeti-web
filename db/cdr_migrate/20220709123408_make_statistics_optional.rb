class MakeStatisticsOptional < ActiveRecord::Migration[6.1]
  def up
    execute %q{
    alter table sys.config add disable_realtime_statistics boolean not null default false;
...
$$;
  ...
  def down
    execute %q{
    alter table sys.config drop column disable_realtime_statistics;
