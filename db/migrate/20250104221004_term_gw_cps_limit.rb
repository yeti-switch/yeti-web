class TermGwCpsLimit < ActiveRecord::Migration[7.0]

  def up
    execute %q{
      alter table class4.gateways
        add termination_cps_limit smallint,
        add termination_cps_wsize smallint not null default 1,
        add termination_subscriber_cps_limit smallint,
        add termination_subscriber_cps_wsize smallint not null default 1;

      alter table switch22.resource_type add rate_limit boolean not null default false;

      insert into class4.disconnect_code (id, namespace_id, code, reason) values (1514, 1, 480,  'Termination gw cps limit');
      insert into class4.disconnect_code (id, namespace_id, code, reason) values (1515, 1, 480,  'Dest. subscriber cps limit');
...
BEGIN
  ...
  */
  ...
  if i_vendor_gw.termination_subscriber_cps_limit is not null then
    i_profile.legb_res = i_profile.legb_res||'10:'||i_vendor_gw.id||'_'||translate(i_profile.dst_prefix_out,': #@', '____')||':'||i_vendor_gw.termination_subscriber_cps_limit||':'||i_vendor_gw.termination_subscriber_cps_wsize||';';
  end if;
...
$_$;
  ...
  def down
    execute %q{
      alter table class4.gateways
        drop column termination_cps_limit,
        drop column termination_cps_wsize,
        drop column termination_subscriber_cps_limit,
        drop column termination_subscriber_cps_wsize;

      alter table switch22.resource_type drop column rate_limit;

      delete from switch22.resource_type where id in (9, 10);
      delete from class4.disconnect_code where id in (1514, 1515);
