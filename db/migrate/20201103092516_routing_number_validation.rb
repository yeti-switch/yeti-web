class RoutingNumberValidation < ActiveRecord::Migration[5.2]
  def up
    execute %q{
      alter table class4.routing_plans
        add validate_dst_number_format boolean not null default false,
        add validate_dst_number_network boolean not null default false;

      INSERT INTO class4.disconnect_code (id, namespace_id, stop_hunting, pass_reason_to_originator, code, reason, rewrited_code, rewrited_reason, success,  successnozerolen,store_cdr,silently_drop) VALUES (8007,0,true,true,404,'No network found for dst number',484,'Address Incomplete',false,false,true,false);
      INSERT INTO class4.disconnect_code (id, namespace_id, stop_hunting, pass_reason_to_originator, code, reason, rewrited_code, rewrited_reason, success,  successnozerolen,store_cdr,silently_drop) VALUES (8008,0,true,true,404,'Invalid dst number format',484,'Address Incomplete',false,false,true,false);

      insert into class4.customers_auth_dst_number_fields(id,name) values(3,'Top-most Diversion header userpart');

CREATE OR REPLACE FUNCTION switch18.route(
  i_node_id integer,
  i_pop_id integer,
...
) RETURNS SETOF switch18.callprofile62_ty
  ...
  def down
    execute %q{
      alter table class4.routing_plans
        drop column validate_dst_number_format,
        drop column validate_dst_number_network;


      delete from  class4.disconnect_code where id in (8007, 8008);
      delete from  class4.customers_auth_dst_number_fields where id=3;
