class AddSampling < ActiveRecord::Migration[7.2]
  def up
    execute %q{

      create table class4.traffic_sampling_rules(
        id smallserial primary key,
        customer_id integer references public.contractors(id),
        customers_auth_id integer references class4.customers_auth(id),
        src_prefix varchar not null default '',
        dst_prefix varchar not null default '',
        dump_level_id smallint not null default 0,
        dump_rate float not null default 0,
        recording_rate float not null default 0
      );
      create index on class4.traffic_sampling_rules using btree(customer_id);
      create index on class4.traffic_sampling_rules using btree(customers_auth_id);
...
**/
    ...
    drop table class4.traffic_sampling_rules;
