class IpAuthOffloading < ActiveRecord::Migration[6.1]
  def up
    execute %q{
...
CREATE or replace FUNCTION switch20.load_ip_auth(
  i_pop_id integer,
  i_node_id integer
) RETURNS TABLE(
  ip inet,
  ...
  require_identity_parsing boolean
)
LANGUAGE plpgsql COST 10 ROWS 100
    AS $$
BEGIN
