...
class GatewayRouteset < ActiveRecord::Migration[7.0]
  def up
    execute %q{

      ALTER TABLE class4.gateways
        ADD COLUMN term_route_set character varying[] NOT NULL DEFAULT '{}',
        ADD COLUMN orig_route_set character varying[] NOT NULL DEFAULT '{}';

      ALTER TABLE class4.gateways RENAME COLUMN term_force_outbound_proxy TO term_force_route_set;
      ALTER TABLE class4.gateways RENAME COLUMN orig_force_outbound_proxy TO orig_force_route_set;

      UPDATE class4.gateways
        SET term_route_set = ARRAY[CASE sip_schema_id WHEN 2 THEN 'sips' ELSE 'sip' END || ':' || term_outbound_proxy || ';transport=udp']
        WHERE term_use_outbound_proxy AND COALESCE(term_outbound_proxy, '') != '' AND term_proxy_transport_protocol_id = 1;

      UPDATE class4.gateways
        SET term_route_set = ARRAY[CASE sip_schema_id WHEN 2 THEN 'sips' ELSE 'sip' END || ':' || term_outbound_proxy || ';transport=tcp']
        WHERE term_use_outbound_proxy AND COALESCE(term_outbound_proxy, '') != '' AND term_proxy_transport_protocol_id = 2;

      UPDATE class4.gateways
        SET term_route_set = ARRAY[CASE sip_schema_id WHEN 2 THEN 'sips' ELSE 'sip' END || ':' || term_outbound_proxy || ';transport=tls']
        WHERE term_use_outbound_proxy AND COALESCE(term_outbound_proxy, '') != '' AND term_proxy_transport_protocol_id = 3;

      UPDATE class4.gateways
        SET orig_route_set = ARRAY[CASE sip_schema_id WHEN 2 THEN 'sips' ELSE 'sip' END || ':' || orig_outbound_proxy || ';transport=udp']
        WHERE orig_use_outbound_proxy AND COALESCE(orig_outbound_proxy, '') != '' AND orig_proxy_transport_protocol_id = 1;

      UPDATE class4.gateways
        SET orig_route_set = ARRAY[CASE sip_schema_id WHEN 2 THEN 'sips' ELSE 'sip' END || ':' || orig_outbound_proxy || ';transport=tcp']
        WHERE orig_use_outbound_proxy AND COALESCE(orig_outbound_proxy, '') != '' AND orig_proxy_transport_protocol_id = 2;
      ...
      ALTER TABLE class4.gateways
        ...
        DROP COLUMN orig_outbound_proxy,
        DROP COLUMN orig_use_outbound_proxy,
        DROP COLUMN term_proxy_transport_protocol_id,
        DROP COLUMN orig_proxy_transport_protocol_id;

      ALTER TABLE data_import.import_gateways
        ADD COLUMN term_route_set character varying[],
        ADD COLUMN orig_route_set character varying[];

      ALTER TABLE data_import.import_gateways RENAME COLUMN term_force_outbound_proxy TO term_force_route_set;
      ALTER TABLE data_import.import_gateways RENAME COLUMN orig_force_outbound_proxy TO orig_force_route_set;
      ...
      ALTER TABLE data_import.import_gateways
        ...
        DROP COLUMN term_proxy_transport_protocol_id,
        DROP COLUMN orig_proxy_transport_protocol_id,
        DROP COLUMN term_proxy_transport_protocol_name,
        DROP COLUMN orig_proxy_transport_protocol_name;

      ALTER TYPE switch22.callprofile_ty
        ADD ATTRIBUTE bleg_route_set character varying,
        ADD ATTRIBUTE aleg_route_set character varying;

CREATE OR REPLACE FUNCTION switch22.new_profile() RETURNS switch22.callprofile_ty
...
$_$;
      ...
      ALTER TYPE switch22.callprofile_ty
        DROP ATTRIBUTE outbound_proxy,
        DROP ATTRIBUTE bleg_outbound_proxy_transport_protocol_id,
        DROP ATTRIBUTE aleg_outbound_proxy,
        DROP ATTRIBUTE aleg_outbound_proxy_transport_protocol_id;

      set search_path TO switch22;
      SELECT * from switch22.preprocess_all();
      set search_path TO gui, public, switch, billing, class4, runtime_stats, sys, logs, data_import;

      DELETE FROM switch22.switch_interface_out WHERE id IN (748, 750, 918, 919);
      INSERT INTO switch22.switch_interface_out (id, name, type, custom, rank, for_radius) VALUES
        (1053, 'bleg_route_set', 'varchar', false, 100, false),
        (1054, 'aleg_route_set', 'varchar', false, 120, false);
  ...
  def down
    execute %q{
      ...
      ALTER TYPE switch22.callprofile_ty
        ADD ATTRIBUTE outbound_proxy character varying,
        ADD ATTRIBUTE bleg_outbound_proxy_transport_protocol_id smallint,
        ADD ATTRIBUTE aleg_outbound_proxy character varying,
        ADD ATTRIBUTE aleg_outbound_proxy_transport_protocol_id smallint;
...
$_$;
      ...
      ALTER TYPE switch22.callprofile_ty
        DROP ATTRIBUTE bleg_route_set,
        DROP ATTRIBUTE aleg_route_set;

      set search_path TO switch22;
      SELECT * from switch22.preprocess_all();
      set search_path TO gui, public, switch, billing, class4, runtime_stats, sys, logs, data_import;

      DELETE FROM switch22.switch_interface_out WHERE id IN (1053, 1054);
      INSERT INTO switch22.switch_interface_out (id, name, type, custom, rank, for_radius) VALUES
        (748, 'outbound_proxy', 'varchar', false, 100, false),
        (750, 'aleg_outbound_proxy', 'varchar', false, 120, false),
        (918, 'aleg_outbound_proxy_transport_protocol_id', 'smallint', false, 121, false),
        (919, 'bleg_outbound_proxy_transport_protocol_id', 'smallint', false, 101, false);

      ALTER TABLE class4.gateways
        ADD COLUMN term_outbound_proxy character varying,
        ADD COLUMN term_use_outbound_proxy boolean NOT NULL DEFAULT false,
        ADD COLUMN orig_outbound_proxy character varying,
        ADD COLUMN orig_use_outbound_proxy boolean NOT NULL DEFAULT false,
        ADD COLUMN term_proxy_transport_protocol_id smallint NOT NULL DEFAULT 1,
        ADD COLUMN orig_proxy_transport_protocol_id smallint NOT NULL DEFAULT 1;

      ALTER TABLE class4.gateways RENAME COLUMN term_force_route_set TO term_force_outbound_proxy;
      ALTER TABLE class4.gateways RENAME COLUMN orig_force_route_set TO orig_force_outbound_proxy;

      ALTER TABLE class4.gateways
        ADD CONSTRAINT gateways_term_proxy_transport_protocol_id_fkey
          FOREIGN KEY (term_proxy_transport_protocol_id) REFERENCES class4.transport_protocols(id),
        ADD CONSTRAINT gateways_orig_proxy_transport_protocol_id_fkey
          FOREIGN KEY (orig_proxy_transport_protocol_id) REFERENCES class4.transport_protocols(id);

      ALTER TABLE class4.gateways
        DROP COLUMN term_route_set,
        DROP COLUMN orig_route_set;

      ALTER TABLE data_import.import_gateways
        ADD COLUMN term_outbound_proxy character varying,
        ADD COLUMN term_use_outbound_proxy boolean,
        ADD COLUMN orig_outbound_proxy character varying,
        ADD COLUMN orig_use_outbound_proxy boolean,
        ADD COLUMN term_proxy_transport_protocol_id smallint,
        ADD COLUMN orig_proxy_transport_protocol_id smallint,
        ADD COLUMN term_proxy_transport_protocol_name character varying,
        ADD COLUMN orig_proxy_transport_protocol_name character varying;

      ALTER TABLE data_import.import_gateways RENAME COLUMN term_force_route_set TO term_force_outbound_proxy;
      ALTER TABLE data_import.import_gateways RENAME COLUMN orig_force_route_set TO orig_force_outbound_proxy;
