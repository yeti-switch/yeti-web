class SrcDstNumberFields < ActiveRecord::Migration[5.2]
  def up
    execute %q{
      ...
      alter table class4.customers_auth
        add src_number_field_id smallint not null default 1 references class4.customers_auth_src_number_fields(id),
        add src_name_field_id smallint not null default 1 references class4.customers_auth_src_name_fields(id),
        add dst_number_field_id smallint not null default 1 references class4.customers_auth_dst_number_fields(id);

      alter table class4.customers_auth_normalized
        add src_number_field_id smallint not null default 1,
        add src_name_field_id smallint not null default 1,
        add dst_number_field_id smallint not null default 1;

      alter table data_import.import_customers_auth
        add src_number_field_id smallint,
        add src_number_field_name varchar,
        add src_name_field_id smallint,
        add src_name_field_name smallint,
        add dst_number_field_id smallint,
        add dst_number_field_name smallint;
      ...
      create table class4.rate_plan_groups(
        id serial primary key,
        rateplan_id integer not null references class4.rateplans(id),
        rate_group_id integer not null references class4.rate_groups(id)
      );
      create unique index on class4.rate_plan_groups using btree(rateplan_id,rate_group_id);


      insert into class4.rate_groups(id, name) select id,name from class4.rateplans;
      select setval('class4.rate_groups_id_seq'::regclass, m ) from (select max(id) + 2 as m from class4.rate_groups) h;

      insert into class4.rate_plan_groups(rateplan_id, rate_group_id) select id, id from class4.rateplans;


      alter table class4.destinations drop constraint destinations_rateplan_id_fkey;
      alter table class4.destinations rename column rateplan_id to rate_group_id;
      alter table class4.destinations add constraint destinations_rate_group_id_fkey FOREIGN KEY(rate_group_id) references class4.rate_groups(id);

      alter table data_import.import_destinations rename column rateplan_id to rate_group_id;
      alter table data_import.import_destinations rename column rateplan_name to rate_group_name;

      alter table class4.rateplans add external_id bigint unique;

      alter table class4.routing_plans
        add external_id bigint unique;

      alter table data_import.import_routing_groups
        drop column sorting_name,
        drop column sorting_id,
        drop column more_specific_per_vendor,
        drop column rate_delta_max;


CREATE OR REPLACE FUNCTION switch18.route(i_node_id integer, i_pop_id integer, i_protocol_id smallint, i_remote_ip inet, i_remote_port integer, i_local_ip inet, i_local_port integer, i_from_dsp character varying, i_from_name character varying, i_from_domain character varying, i_from_port integer, i_to_name character varying, i_to_domain character varying, i_to_port integer, i_contact_name character varying, i_contact_domain character varying, i_contact_port integer, i_uri_name character varying, i_uri_domain character varying, i_auth_id integer, i_x_yeti_auth character varying, i_diversion character varying, i_x_orig_ip inet, i_x_orig_port integer, i_x_orig_protocol_id smallint, i_pai character varying, i_ppi character varying, i_privacy character varying, i_rpid character varying, i_rpid_privacy character varying) RETURNS SETOF switch18.callprofile62_ty
  ...
  def down
    execute %q{
      alter table class4.customers_auth
        drop column src_number_field_id,
        drop column src_name_field_id,
        drop column dst_number_field_id;
      ...
      alter table data_import.import_customers_auth
        drop column src_number_field_id,
        drop column src_number_field_name,
        drop column src_name_field_id,
        drop column src_name_field_name,
        drop column dst_number_field_id,
        drop column dst_number_field_name;
      ...
      alter table class4.destinations drop constraint destinations_rate_group_id_fkey;
      alter table class4.destinations rename column rate_group_id to rateplan_id;
      alter table class4.destinations add constraint destinations_rateplan_id_fkey FOREIGN KEY(rateplan_id) references class4.rateplans(id);

      alter table data_import.import_destinations rename column rate_group_id to rateplan_id;
      alter table data_import.import_destinations rename column rate_group_name to rateplan_name;

      drop table class4.rate_plan_groups;
      drop table class4.rate_groups;
      drop table data_import.import_rate_groups;

      alter table class4.rateplans drop column external_id;
      alter table class4.routing_plans drop column external_id;

      alter table data_import.import_routing_groups
        add sorting_name varchar,
        add sorting_id integer,
        add more_specific_per_vendor boolean,
        add rate_delta_max numeric not null default 0;
