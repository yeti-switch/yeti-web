...
class GatewayPidfloMode < ActiveRecord::Migration[7.2]
  def up
    execute %q{
      alter table class4.gateways add pidflo_mode_id smallint not null default 0;
      alter table data_import.import_gateways add pidflo_mode_id smallint;
      alter table data_import.import_gateways add pidflo_mode_name varchar;

      alter type switch22.callprofile_ty
        add attribute pidflo_mode_id smallint;
...
set search_path TO gui, public, switch, billing, class4, runtime_stats, sys, logs, data_import;
      ...
      delete from switch22.switch_interface_out where name = 'pidflo_mode_id';

      alter type switch22.callprofile_ty
        drop attribute pidflo_mode_id;

      alter table data_import.import_gateways drop column pidflo_mode_name;
      alter table data_import.import_gateways drop column pidflo_mode_id;
      alter table class4.gateways drop column pidflo_mode_id;
    }
  end
end
