...
class AddCustomerVendorCurrencyId < ActiveRecord::Migration[7.2]
  def up
    execute %q{
      alter type switch22.callprofile_ty
        add attribute customer_currency_id smallint,
        add attribute vendor_currency_id smallint,
        add attribute customer_currency_rate double precision,
        add attribute vendor_currency_rate double precision;
      ...
      ALTER TABLE class4.destinations ADD COLUMN currency_id smallint REFERENCES billing.currencies(id);
      UPDATE class4.destinations SET currency_id = 0;
      ALTER TABLE class4.destinations ALTER COLUMN currency_id SET NOT NULL;

      ALTER TABLE data_import.import_destinations ADD COLUMN currency_id smallint;
    }
...
**/
    ...
    execute %q{
      ALTER TABLE data_import.import_destinations DROP COLUMN currency_id;
      ALTER TABLE class4.destinations DROP COLUMN currency_id;

      delete from switch22.switch_interface_out where name in ('customer_currency_id', 'vendor_currency_id', 'customer_currency_rate', 'vendor_currency_rate');

      alter type switch22.callprofile_ty
        drop attribute customer_currency_id,
        drop attribute vendor_currency_id,
        drop attribute customer_currency_rate,
        drop attribute vendor_currency_rate;
    }
  end
end
