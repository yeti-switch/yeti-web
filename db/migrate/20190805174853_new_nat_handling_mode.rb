...
#
  def up
    ...
    CREATE SCHEMA switch19;
      ...
      CREATE OR REPLACE FUNCTION switch19.lua_clear_cache()
      RETURNS void
      LANGUAGE 'pllua'
      COST 100
      VOLATILE AS
      ...
      CREATE OR REPLACE FUNCTION switch19.lua_exec(
        function_id integer,
        arg switch19.lua_call_context
      ) RETURNS switch19.lua_call_context
      LANGUAGE 'pllua'
      COST 100
      VOLATILE AS
      $BODY$
...
CREATE TYPE switch19.callprofile62_ty AS (
...
CREATE FUNCTION switch19.debug(i_transport_protocol_id smallint, i_remote_ip inet, i_remote_port integer, i_src_prefix character varying, i_dst_prefix character varying, i_pop_id integer, i_uri_domain character varying, i_from_domain character varying, i_to_domain character varying, i_x_yeti_auth character varying, i_release_mode boolean DEFAULT false, i_pai character varying DEFAULT NULL::character varying, i_ppi character varying DEFAULT NULL::character varying, i_privacy character varying DEFAULT NULL::character varying, i_rpid character varying DEFAULT NULL::character varying, i_rpid_privacy character varying DEFAULT NULL::character varying) RETURNS SETOF switch19.callprofile62_ty
...
DECLARE
  ...
  v_end timestamp;
BEGIN
  set local search_path to switch19,sys,public;
  v_start:=now();
...
CREATE or replace FUNCTION switch19.load_incoming_auth() RETURNS TABLE(id integer, username character varying, password character varying)
...
CREATE FUNCTION switch19.new_profile() RETURNS switch19.callprofile62_ty
    LANGUAGE plpgsql COST 10
    AS $_$
DECLARE
  v_ret switch19.callprofile62_ty;
BEGIN
...
CREATE FUNCTION switch19.process_dp(i_profile switch19.callprofile62_ty, i_destination class4.destinations, i_dp class4.dialpeers, i_customer_acc billing.accounts, i_customer_gw class4.gateways, i_vendor_acc billing.accounts, i_pop_id integer, i_send_billing_information boolean, i_max_call_length integer) RETURNS SETOF switch19.callprofile62_ty
...
CREATE FUNCTION switch19.process_gw(i_profile switch19.callprofile62_ty, i_destination class4.destinations, i_dp class4.dialpeers, i_customer_acc billing.accounts, i_customer_gw class4.gateways, i_vendor_acc billing.accounts, i_vendor_gw class4.gateways, i_send_billing_information boolean, i_max_call_length integer) RETURNS switch19.callprofile62_ty
...
SELECT pg_catalog.setval('switch19.switch_interface_id_seq', 1034, true);
