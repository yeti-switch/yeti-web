...
class DiversionSrcNumberlist < ActiveRecord::Migration[6.1]
  def up
    execute %q{

DROP FUNCTION switch20.load_disconnect_code_refuse_overrides();
CREATE FUNCTION switch20.load_disconnect_code_refuse_overrides()
 RETURNS TABLE(policy_id integer, o_id integer, o_code integer, o_reason character varying, o_rewrited_code integer, o_rewrited_reason character varying, o_store_cdr boolean, o_silently_drop boolean)
 LANGUAGE plpgsql
 COST 10
AS $function$
BEGIN
...
$function$;
        ...
        alter table class4.customers_auth add src_numberlist_use_diversion boolean not null default false;
        alter table class4.customers_auth_normalized add src_numberlist_use_diversion boolean not null default false;

CREATE FUNCTION switch20.match_numberlist(
  i_numberlist_id integer,
  i_key varchar,
  i_key2 varchar default null
) RETURNS class4.numberlist_items
 LANGUAGE plpgsql
 COST 10
AS $function$
DECLARE
...
**/
  ...
  def down
    execute %q{
        alter table class4.customers_auth drop column src_numberlist_use_diversion;
        alter table class4.customers_auth_normalized drop column src_numberlist_use_diversion;
...
**/
    ...
    delete from switch20.switch_interface_out where id in(1039,1040);
...
DROP FUNCTION switch20.load_disconnect_code_refuse_overrides();
CREATE FUNCTION switch20.load_disconnect_code_refuse_overrides()
 RETURNS TABLE(o_policy_id integer, o_id integer, o_code integer, o_reason character varying, o_rewrited_code integer, o_rewrited_reason character varying, o_store_cdr boolean, o_silently_drop boolean)
 LANGUAGE plpgsql
 COST 10
AS $function$
BEGIN
...
drop FUNCTION switch20.match_numberlist(integer,varchar,varchar);
