  ...
  def up
    execute %q{
      alter table class4.gateways
        add pai_send_mode_id smallint not null default 0,
        add pai_domain varchar,
        add registered_aor_mode_id smallint not null default 0;

      update class4.gateways set registered_aor_mode_id=1 where use_registered_aor;
      alter table class4.gateways drop column use_registered_aor;

alter type switch20.callprofile_ty add attribute registered_aor_mode_id smallint;
...
$_$;
  ...
  def down
    execute %q{
      alter table class4.gateways
        drop column pai_send_mode_id,
        drop column pai_domain;
      alter table class4.gateways add use_registered_aor boolean not null default false;

      update class4.gateways set use_registered_aor=true where registered_aor_mode_id>0;
      alter table class4.gateways drop column registered_aor_mode_id;
