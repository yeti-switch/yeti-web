class UpgradeYetiExt < ActiveRecord::Migration[6.1]
  def up
    execute %q{

INSERT INTO class4.disconnect_code (id, namespace_id, stop_hunting, pass_reason_to_originator, code, reason, rewrited_code, rewrited_reason, success,  successnozerolen,store_cdr,silently_drop) VALUES (8009,0,true,true,503,'No response from CNAM DB', NULL, NULL, false, false, true, false);
...
CREATE OR REPLACE FUNCTION switch20.cnam_lua_response_exec(fn varchar, arg text)
 RETURNS switch20.cnam_lua_resp
 LANGUAGE pllua
AS $function$
        func = assert(load(fn))
        return func()(arg)
...
CREATE OR REPLACE FUNCTION switch20.cnam_lua_build_request(fn varchar, arg text)
 RETURNS text
 LANGUAGE pllua
AS $function$
        func = assert(load(fn))
...
**/
  ...
  drop FUNCTION switch20.cnam_lua_response_exec(fn varchar, arg text);
  drop FUNCTION switch20.cnam_lua_build_request(fn varchar, arg text);

  drop type switch20.cnam_lua_resp;
    ...
    delete from class4.disconnect_code where id=8009;
