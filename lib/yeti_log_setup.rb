...
# VictoriaLogs when config/yeti_web.yml configures it.
#
...
# stdout from config/application.rb, so that the boot is logged too, VictoriaLogs from
# config/initializers/semantic_logger.rb, the first point where YetiConfig is loaded.
module YetiLogSetup
  # Applied when `logging.victorialogs.batch_size` is not configured, see .batch_options.
  ...
  # Logs to stdout in the plain one line format and to VictoriaLogs, when it is configured.
  #
  ...
  def call(component:, level: :info, tags: {})
    ...
    begin
      ...
      # A process may have no other use for config/yeti_web.yml, and an invalid url
      # makes the appender raise anything from URI::InvalidURIError to ArgumentError.
      # None of that may stop the process.
      logger.warn "Logging to VictoriaLogs is disabled: #{e.class}: #{e.message}"
    end
  ...
  # Applies `logging.stdout.level` and `logging.victorialogs.level`. An appender level
  ...
  # @param tags [Hash] extra static tags, see .add_victorialogs_appender.
  # @return [SemanticLogger::Subscriber, nil] the VictoriaLogs appender.
  def apply_levels!(default_level:, tags: {})
    stdout_level, victorialogs_level = configured_levels
    fallback = default_level.to_s.downcase.to_sym

    if stdout_level.nil? && victorialogs_level.nil?
      ...
      victorialogs_level ||= fallback
      SemanticLogger.default_level = most_verbose(stdout_level, victorialogs_level)
    end

    # Found and not passed: config/application.rb registers the stdout appender. The Async
    # proxy of the VictoriaLogs appender delegates #console_output? to it, and it is false.
    ...
    add_victorialogs_appender(tags:, level: victorialogs_level)
  end
  ...
  # Requires YetiConfig to be loaded.
  #
  # @return [Array<Symbol, nil>] two elements, the stdout and the VictoriaLogs level,
  #   nil for an appender that is given none of its own.
  def configured_levels
    logging = YetiConfig.logging
    [logging&.stdout&.level, logging&.victorialogs&.level].map { |level| level&.to_s&.downcase&.to_sym }
  end
  ...
  # `logging.victorialogs.url` is what disables the logging to VictoriaLogs.
  #
  # The ingestion settings of VictoriaLogs are the query string of the url, for example
  # `?_msg_field=message&_time_field=timestamp&_stream_fields=host,component,system`.
  #
  # @param tags [Hash] extra static tags, added to YetiConfig.logging.victorialogs.tags.
  ...
  #   the YetiVictoriaLogsAppender that writes them. nil when it is not configured.
  def add_victorialogs_appender(tags: {}, level: nil)
    config = YetiConfig.logging&.victorialogs
    return if config.blank? || config.url.blank?

    # Not at the top of the file: bin/cdr_processor has no other use for net/http.
    require_relative 'yeti_victorialogs_appender'
    ...
    appender = YetiVictoriaLogsAppender.new(
      url: config.url,
      level:,
  ...
  #
  # `batch` is passed explicitly: Appender.factory only batches by default when the
  # appender opts in through #batch_by_default?, and Appender::Http does not - without it
  # the records are posted one by one from the thread that logged them.
  #
  # @param config [Config::Options] YetiConfig.logging.victorialogs
  # @return [Hash]
