...
#
# ajax_tab.js lazy-loads a tab's content the first time it is shown, and
# chart_init.js binds each chart's render to the tab click (the charts declare
# their tab in `data-chart-tab`). Flowbite has no DOM event for either, so each
# tab button carries `data-tab-target` and both hook off it. Keep the three in
# sync.
module ActiveAdmin
  module Views
