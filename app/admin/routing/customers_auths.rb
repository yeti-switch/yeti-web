...
ActiveAdmin.register CustomersAuth do
  ...
  filter :diversion_policy_id_eq, label: 'Diversion policy', as: :select, collection: CustomersAuth::DIVERSION_POLICIES.invert, input_html: { class: 'tom-select' }
  filter :pai_policy_id_eq, label: 'PAI policy', as: :select, collection: CustomersAuth::PAI_POLICIES.invert, input_html: { class: 'tom-select' }
  ...
  filter :transport_protocol_id_eq, label: 'Transport protocol', as: :select, collection: CustomersAuth::TRANSPORT_PROTOCOLS.invert, input_html: { class: 'tom-select' }
  filter :ip_covers,
         as: :string,
  ...
  form do |f|
    ...
    tabs do
      ...
      tab :stir_shaken do
        f.inputs do
          f.input :ss_mode_id, as: :select, include_blank: false, collection: CustomersAuth::SS_MODES.invert, input_html: { class: 'tom-select' }
          f.input :ss_invalid_identity_action_id, as: :select, include_blank: false, collection: CustomersAuth::SS_INVALID_IDENTITY_ACTIONS.invert, input_html: { class: 'tom-select' }
          f.input :ss_no_identity_action_id, as: :select, include_blank: false, collection: CustomersAuth::SS_NO_IDENTITY_ACTIONS.invert, input_html: { class: 'tom-select' }
          f.input :rewrite_ss_status_id, as: :select, include_blank: true, collection: Equipment::StirShaken::Attestation::ATTESTATIONS.invert, input_html: { class: 'tom-select' }
