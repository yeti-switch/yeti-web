...
class InvoiceDocs
  ...
  def save!
    raise TemplateUndefined.new("Template blank for invoice: #{invoice.id}") if template.blank?
    #?
  ...
  def save_invoice!
    ...
    #generate pdf
    Open3.popen3("HOME=/home/yeti-web /usr/bin/unoconv -f pdf #{Shellwords.escape(odf_path)}") { |stdin, stdout, stderr, wait_thr|
