...
class Stats::Traffic < Stats::Base
  ...
  def self.to_chart(id, _options = {})
    points = where(account_id: id)
             .where('timestamp <= NOW() AND timestamp > ?', 12.hours.ago).order('timestamp asc')
             .pluck(:timestamp, :amount, :profit)
    [
      {
        key: 'Profit',
        values: points.map { |p| { x: p[0].to_i * 1000, y: p[2].to_f } }
      },
      {
        key: 'Amount',
        values: points.map { |p| { x: p[0].to_i * 1000, y: p[1].to_f } }
      }
  ...
  def self.stats(column, hours_ago = 48)
    ...
    res = [
      ...
      values: scope.map { |p| { x: p[:timestamp].to_i * 1000, y: p[column].to_f } }
    ]
    res
  end
end
