...
# == Schema Information
#
# Table name: billing.invoice_terminated_networks
#
...
# Indexes
#
#  invoice_terminated_networks_invoice_id_idx  (invoice_id)
#
# Foreign Keys
#
#  invoice_terminated_networks_invoice_id_fkey  (invoice_id => invoices.id)
#

class Billing::InvoiceTerminatedNetwork < Cdr::Base
  self.table_name = 'billing.invoice_terminated_networks'
  ...
  scope :for_invoice_succ, -> { preload(:country, :network).order(:spent, :country_id).where('successful_calls_count>0') }
